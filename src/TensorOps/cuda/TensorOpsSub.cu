...
template<typename T>
__global__ void sub_kernel_nd_broadcast(const T* a, const T* b, T* output,
...
template<>
__global__ void sub_kernel_nd_broadcast<__half>(const __half* a, const __half* b, __half* output,
                                              const size_t* a_shape, const size_t* b_shape, const size_t* out_shape,
                                              const size_t* a_strides, const size_t* b_strides, const size_t* out_strides,
                                              size_t a_ndim, size_t b_ndim, size_t out_ndim,
                                              size_t total_elems)
{
    ...
    output[linear_idx] = __hsub(a[a_idx], b[b_idx]);
}

template<>
__global__ void sub_kernel_nd_broadcast<__nv_bfloat16>(const __nv_bfloat16* a, const __nv_bfloat16* b, __nv_bfloat16* output,
                                                    const size_t* a_shape, const size_t* b_shape, const size_t* out_shape,
                                                    const size_t* a_strides, const size_t* b_strides, const size_t* out_strides,
                                                    size_t a_ndim, size_t b_ndim, size_t out_ndim,
                                                    size_t total_elems)
...
void cuda_sub_tensor(const Tensor& A, const Tensor& B, Tensor& output, cudaStream_t stream)
{
    ...
    dispatch_by_dtype(A.dtype(), [&](auto dummy)
    {
        ...
        if (!needs_broadcasting) {
            ...
            sub_kernel_nd_broadcast<<<grid_size, block_size, 0, stream>>>(
...
template <typename T>
    ...
    template <typename T>
    __global__ void sub_inplace_kernel_broadcast(T* lhs, const T* rhs,
    ...
    template <>
    __global__ void sub_inplace_kernel_broadcast<__half>(__half* lhs, const __half* rhs,
                                                       size_t lhs_rows, size_t lhs_cols,
                                                       size_t rhs_rows, size_t rhs_cols,
                                                       size_t out_rows, size_t out_cols)
    {
        ...
        if (idx < total_elems) {
            ...
            lhs[lhs_idx] = __hsub(lhs[lhs_idx], rhs[rhs_idx]);
        }
    }

    template <>
    __global__ void sub_inplace_kernel_broadcast<__nv_bfloat16>(__nv_bfloat16* lhs, const __nv_bfloat16* rhs,
                                                              size_t lhs_rows, size_t lhs_cols,
                                                              size_t rhs_rows, size_t rhs_cols,
                                                              size_t out_rows, size_t out_cols)
    {
