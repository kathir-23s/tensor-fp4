...
template<typename T>
__global__ void bool_and_kernel(const T* a, const T* b, bool* output, size_t n)
{
...
template<>
__global__ void bool_and_kernel<__half>(const __half* a, const __half* b, bool* output, size_t n)
{
    size_t idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx < n)
    {
...
template<>
__global__ void bool_and_kernel<__nv_bfloat16>(const __nv_bfloat16* a, const __nv_bfloat16* b, bool* output, size_t n)
{
    size_t idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx < n)
    {
...
template<>
__global__ void bool_and_kernel_broadcast<__nv_bfloat16>(const __nv_bfloat16* a, const __nv_bfloat16* b, bool* output,
                                      const size_t* a_shape, const size_t* b_shape, const size_t* out_shape,
                                      const size_t* a_strides, const size_t* b_strides, const size_t* out_strides,
                                      size_t a_ndim, size_t b_ndim, size_t out_ndim,
                                      size_t total_elems)
...
void cuda_logical_and_outplace(const Tensor &A, const Tensor &B, Tensor &output, cudaStream_t stream)
{
    ...
    dispatch_by_dtype(A.dtype(), [&](auto dummy)
    {
        ...
        if (!needs_broadcasting) {
            bool_and_kernel<<<grid_size, block_size, 0, stream>>>(a_ptr, b_ptr, output_ptr, total_elems);
        } else {
            ...
            bool_and_kernel_broadcast<<<grid_size, block_size, 0, stream>>>(
